...
export class Blank {
  ...
  private _blank_no: string;
  private _answer: string;
  public resolved: any;
  public solution: any;
  public targetable: any;
  public targetingOptionGroup: any;
  public xpath: any[];

  constructor(question: any, blank_no: string) {
  ...
  setTarget() {
    ...
    if (!optionGroup) {
      console.log('题空%d没有选项', this.blankNo);
      return;
    }
...
export function collectTargingBlankCandidates(question: any) {

  let candidates = [];
  for (let blank_no in question._blanks) {
  ...
  for (let subquestion of question.subquestions) {
    for (let blank of subquestion._boundBlanks) {
  ...
  for (let blank of question._boundBlanks) {
