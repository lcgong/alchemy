...
export function plugin(md: any) {
  ...
  md.renderer.rules.math = function (tokens: any, idx: number) {
    return escapeHtml(tokens[idx].content)
  }
  md.renderer.rules.inline_math = function (tokens: any, idx: number) {
    return '\\(' + escapeHtml(tokens[idx].content) + '\\)'
  }
  md.renderer.rules.display_math = function (tokens: any, idx: number) {
    return '\\[' + escapeHtml(tokens[idx].content) + '\\]'
  }
}
